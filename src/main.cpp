...
unsigned int nStakeMinAge = 60 * 60 * 24 * 30*100; // minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90*100; // stake age of full weight
unsigned int nStakeTargetSpacing = 2 * 60; // DIFF: 1-minute block spacing
int64 nChainStartTime = 1390815813;
int64 nNextStageTime = 1418158920;
...
static int64 nRewardCoinYearMin = 0 * CENT;  // creation amount per coin-year
static int64 nRewardCoinYearMax = 0 * CENT;
...
const string strMessageMagic = "HeroCoin Signed Message:\n";
...
// herocoin: increasing Nfactor gradually
...
int64 GetProofOfWorkReward(unsigned int nBits, int nHeight)
{
      if(nHeight<=1)
       {
       return MAX_MINT_PROOF_OF_WORK*116666;//100000;
       }
      else if(nHeight<=43200){
      return MAX_MINT_PROOF_OF_WORK;
      }
       else{
           int nSubsidy=std::floor( nHeight/43200.00);
           //powMax = MAX_MINT_PROOF_OF_WORK;
            int64  powMax=std::pow(0.5,nSubsidy) * MAX_MINT_PROOF_OF_WORK;
           return max(powMax, MIN_MINT_PROOF_OF_WORK);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in HeroCoin
    bool fStrictPayToScriptHash = true; // Always active in HeroCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HeroCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "XHU5NTdGXHU2Qzk5XHU5MDFBXHU2NjEzXHU4RjZGXHU0RUY2XHU3OUQxXHU2MjgwXHU2NzA5XHU5NjUwXHU1MTZDXHU1M0Y4 ";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xdcdb0261e37b230fd6ac012c80c9fb4815cc499d7cfb2b93c8b513c3dce30ddb"));
        block.print();
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    { Sleep(20000);
         // printf("%d vvvvvvvvvvvvvtestzhang\n", vNodes.size());
        if( vNodes.size()<=0)
            return;
